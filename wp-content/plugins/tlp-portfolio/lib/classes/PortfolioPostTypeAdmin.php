...
if ( ! class_exists( 'PortfolioPostTypeAdmin' ) ) :
	...
	class PortfolioPostTypeAdmin {
		...
		public function manage_portfolio_sc_columns( $column ) {
			switch ( $column ) {
				case 'shortcode':
					echo '<input type="text" onfocus="this.select();" readonly="readonly" value="[tlpportfolio id=&quot;' . esc_attr(get_the_ID()) . '&quot; title=&quot;' . esc_html( get_the_title() ) . '&quot;]" class="large-text code tlp-code-sc">';
					break;
				default:
					break;
			}
		...
		protected function build_taxonomy_filter( $tax_slug ) {
			...
			$current_tax_slug = isset( $_GET[ $tax_slug ] ) ? sanitize_title_with_dashes( wp_unslash( $_GET[ $tax_slug ] ) ) : false; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
