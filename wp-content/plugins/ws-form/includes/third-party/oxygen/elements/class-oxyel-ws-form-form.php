<?php
	...
	class OxyEl_WS_Form_Form extends OxyEl {
		...
		public function render($options, $defaults, $content) {
			...
			// Read options
			$form_id = absint(isset($options['wsf_form_id']) ? $options['wsf_form_id'] : 0);
				...
				if($this->is_oxygen_element) {
					...
					add_filter('wsf_public_enqueue', function($enqueue) {
						...
						return (
							...
							isset($_REQUEST) &&										// phpcs:ignore WordPress.Security.NonceVerification
							isset($_REQUEST['action']) &&							// phpcs:ignore WordPress.Security.NonceVerification
							($_REQUEST['action'] === 'oxy_render_oxy-wsform_form')	// phpcs:ignore WordPress.Security.NonceVerification
