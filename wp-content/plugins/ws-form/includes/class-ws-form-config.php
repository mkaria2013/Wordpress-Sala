				...
				'basic' => array(
					...
					'types' => array(
						...
						'textarea' => array (
							...
							'mask_field_attributes'			=>	array('class', 'disabled', 'readonly', 'required', 'min_length', 'max_length', 'min_length_words', 'max_length_words', 'input_mask', 'input_mask_validate', 'placeholder', 'spellcheck', 'cols', 'rows', 'aria_describedby', 'aria_labelledby', 'aria_label', 'custom_attributes', 'hidden_bypass', 'autocomplete', 'transform', 'inputmode'),
							'mask_field_label'				=>	'<label id="#label_id" for="#id"#attributes>#label</label>',
							'mask_field_label_attributes'	=>	array('class'),
		...
		public static function get_meta_keys($form_id = 0, $public = false) {
			...
			$meta_keys = array(
				...
				'invalid_feedback_mask' => array(
					...
					'type'						=>	'text',
					'default'					=>	'',
					'mask_placeholder'			=>	apply_filters('wsf_field_invalid_feedback_text', WS_FORM_FIELD_INVALID_FEEDBACK),
					'help'						=>	sprintf(
			...
			// IDs to include (also includes any schedule IDs beginning with wsf_)
		...
		public static function get_parse_variables($public = true) {
			...
			$parse_variables = array(
				...
				'server'	=>	array(
					...
					'variables'	=> array(
						...
						'server_time' => array('label' => __('Current Time', 'ws-form'), 'value' => $wp_new ? wp_date(get_option('time_format'), time(), new DateTimeZone(date_default_timezone_get())) : gmdate(get_option('time_format')), 'description' => __('Returns the server time in the format configured in WordPress.', 'ws-form'), 'secure' => true),
						...
						'server_date' => array('label' => __('Current Date', 'ws-form'), 'value' => $wp_new ? wp_date(get_option('date_format'), time(), new DateTimeZone(date_default_timezone_get())) : gmdate(get_option('date_format')), 'description' => __('Returns the server date in the format configured in WordPress.', 'ws-form'), 'secure' => true)
					)
 				),
				...
				'submit' 		=> array(
					...
					'variables'	=> array(
						...
						'submit_date_added'		=>	array('label' => __('Date Added', 'ws-form'), 'description' => __('Returns the date and time the submission occurred.', 'ws-form'), 'secure' => true),
						'submit_date_added_custom'		=>	array(
							...
							'attributes' => array(
								...
								array('id' => 'format', 'required' => true)
							),

							'kb_slug' => 'date-formats',

							'description' => __('Returns the submit date and time in a specified format (PHP date format).', 'ws-form'),
			...
			$parse_variables['post'] = array(
				...
				'variables'	=> array(
					...
					'post_time'			=>	array('label' => __('Time', 'ws-form'), 'value' => $post_not_null ? ($wp_new ? wp_date(get_option('time_format'), strtotime($post->post_date_gmt)) : gmdate(get_option('time_format'), strtotime($post->post_date))) : '', 'secure' => true),
					...
					'post_date'			=>	array('label' => __('Date', 'ws-form'), 'value' => !is_null($post) ? ($wp_new ? wp_date(get_option('date_format'), strtotime($post->post_date_gmt)) : gmdate(get_option('date_format'), strtotime($post->post_date))) : '', 'secure' => true),
			...
			// ACF - Discontinuing in favor of post management add-on
/*			if(class_exists('acf')) { 
